...
if (mysqli_num_rows($result) > 0) {
    ...
    $users = '<table class="table table-bordered">
        <tr>
            ...
            <th>Name</th>
            <th>Student Task</th>
            <th>Performance Interns</th>
            <th>Interns New Task</th>
        </tr>
    ';
    while ($row = mysqli_fetch_assoc($result)) {
        $users .= '<tr>
            <td>'.$number.'</td>
            <td>'.$row['Name'].'</td>
            <td>'.$row['Student_task'].'</td>
            <td>'.$row['Performance_Interns'].'</td>
            <td>'.$row['Interns_new_task'].'</td>
        </tr>';
        $number++;
    }
    $users .= '</table>';
...
<div class="container">
    ...
    <script>
        function Export()
        {
            ...
            if(conf == true)
            {
                window.open("CSWeeklyexport.php", '_blank');
            }
        }
    </script>
</div>
